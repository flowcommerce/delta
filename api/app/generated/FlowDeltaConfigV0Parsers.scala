 ...
 * apibuilder 0.14.59 app.apibuilder.io/flow/delta-config/0.7.61/anorm_2_6_parsers
 */
import anorm._
  ...
  object Build {
    ...
    def parser(
      ...
      SqlParser.bool(prefixOpt.getOrElse("") + crossZoneLoadBalancing).? map {
        case name ~ dockerfile ~ initialNumberInstances ~ instanceType ~ memory ~ containerMemory ~ portContainer ~ portHost ~ remoteLogging ~ stages ~ dependencies ~ version ~ healthcheckUrl ~ crossZoneLoadBalancing => {
